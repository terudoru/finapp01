...
# --- サイドバー (入力情報のUI) ---
# --- サイドバー (入力情報のUI) ---
app_mode = st.sidebar.radio("🔍 アプリモード", ["📊 個別銘柄の詳細分析", "🧪 バックテスト・検証機能", "🏆 AI 一斉スクリーナー (買い/売り)", "💡 おすすめ銘柄推薦", "💼 ポートフォリオ管理", "⚙️ 設定"])
st.sidebar.markdown("---")
...
elif app_mode == "💡 おすすめ銘柄推薦":
    run_button = st.sidebar.button("🌟 おすすめ銘柄を抽出する", type="primary")
elif app_mode == "⚙️ 設定":
    run_button = False
else:
...
if app_mode == "💡 おすすめ銘柄推薦":
    st.header(f"💡 AI＆アナリスト おすすめ銘柄推薦 (対象期間: {timeframe})")
    
    col_filter_rec, = st.columns(1)
    with col_filter_rec:
        only_bookmarks_rec = st.checkbox("⭐ マイ・ブックマークの銘柄のみ対象にする", value=False, key="rec_only_book")
        
    st.markdown("XGBoostによる**短期的な上昇確率(>50%)**と、アナリストの目標株価に基づく**中長期的な上値余地**を組み合わせ、「一定期間後に利益が見込める」買い推奨銘柄を厳選します。")
    if run_button:
        with st.spinner("🌟 おすすめ銘柄を分析・抽出中..."):
            all_tickers = []
            
            if only_bookmarks_rec:
                for name, tic in WATCHLIST["⭐ マイ・ブックマーク"].items():
                    cat = "🪙 仮想通貨" if "USDT" in tic or "USD" in tic else "ブックマーク"
                    all_tickers.append({"name": name, "ticker": tic, "category": cat})
            else:
            ...
            for i, item in enumerate(all_tickers):
                tic = item["ticker"]
                is_cry = ("仮想通貨" in item["category"])
                status_text.text(f"分析中 ({i+1}/{len(all_tickers)}): {item['name']}")
                ...
                try:
                    ...
                    if not is_cry:
                        try:
                            ...
                            recom_key = info.get("recommendationKey", "none")
                            analyst_recom = recom_key.replace("_", " ").title()
                            
                            if t_mean:
                                target_price_str = f"${t_mean:,.2f}"
                                if curr_price > 0:
                                    upside = (t_mean / curr_price) - 1.0
                        except: pass
                    else:
